...
private _ehOld = GETMVAR(vnd_osdEH, -1);
...
private _eh = addMissionEventHandler [
    "Draw3D",
    {
        ...
        if (isNull _uav || !(GETMVAR(vnd_isControl, false))) exitWith {
        ...
        private _showHorizon = GETMVAR(vnd_showHorizon, true);
        ...
        if (!isNull _l && !isNull _r && _showHorizon) then {
            private _base = GETMVAR(vnd_hBasePos, []);
            if (_base isEqualTo [] || {count _base != 2}) then {
                _base = [ctrlPosition _l, ctrlPosition _r];
                SETMVAR(vnd_hBasePos, _base);
            };

            private _k = GETMVAR(vnd_hCoef, (pixelGridNoUIScale * pixelH * 2) * 0.20);
